...
export class AddCalibModalComponent implements OnInit {
  ...
  createCalib() {
    this.addCalibForm.value.Start_date = this.dateToString(
      this.addCalibForm.value.Start_date
    );
    this.addCalibForm.value.End_date = this.dateToString(
      this.addCalibForm.value.End_date
    );
    console.log(this.addCalibForm.value);

    this.restService
      .getPosts('create_calib', this.authService.getToken(), {
