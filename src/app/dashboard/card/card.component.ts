...
export class CardComponent implements OnInit {
      ...
      .subscribe({
        next: (data) => {
          ...
          if (data['status'] == 200) {
            ...
            this.tab = [];
            const idfile = [
              ...Array.from(new Set(this.temp2.map((i) => i.Asset_no))),
            ];
            // console.log(idfile)
            idfile.forEach((Asset_no) => {
              let res = this.temp2.filter(
                (Calib_no) => Calib_no.Asset_no == Asset_no
              );
              let res2 = this.temp2.filter(
                (Start_date) => Start_date.Asset_no == Asset_no
              );
              let res3 = this.temp2.filter(
                (End_date) => End_date.Asset_no == Asset_no
              );
              let res4 = this.temp2.filter(
  ...
  filterData() {
    let val = 'missing';
    let temp = this.masterData.filter(
      (temp) => temp.Remarks.toLowerCase().indexOf(val) !== -1 || !val
    );
